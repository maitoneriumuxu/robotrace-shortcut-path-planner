...
- 大域DAGの辺は必ず`start_index < end_index`とする。最終経路の`source_progress_index`は線形補間や`maximum.accumulate`で作らず、各姿勢で実際に接触した白線segment集合から単調DPで求める。
...
- 最終採用には、出典と原点が確認済みのLN5実車床面投影外形を必須とする。最寄り白線距離だけでは合法判定しない。
- 白線幅19mmの領域と、2mm以下かつyaw変化1deg以下へ細分した全姿勢の実車外形を交差判定する。白線から完全離脱するcandidateは即不合格とする。
- 未来ラインへの乗り換えは、実接触segmentの同時接触または空集合を挟まない連続接触をDPで確認する。接触していないsegmentへ進行indexを割り当てない。
- 走行可能領域と白線重なりを分けて評価する。板内だけでは合格にせず、他ライン交差の警告やペナルティを合法性の代わりにしない。
- LN5実車外形が不明・未確認なら新しい大域経路を採用せず、現在のFrenet長窓4.471秒経路へフォールバックする。
...
- 2025年全日本は`outputs/result.png`へ、原コース、現在経路、競技無効な理論下限、LN5実車合法reference、embedded-lite、最終経路を表示する。
- 無効な最大ジャンプ、最小接触余裕位置、乗り換え位置、最大yaw・曲率位置を拡大し、外形確認済みの場合だけ実車床面投影と白線重なりを描く。
- 接触状態図へ実接触segment、DPで選んだsource progress、重なり面積、接触余裕、同時接触ライン数を表示する。未評価値を0として描かない。
- 比較表へ予測時間、合法性、robust警告、完全離脱回数、最小重なり・余裕、乗り換え数、交差リスク、板境界、外形ソース、計算時間を表示する。
