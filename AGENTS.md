...
- AALP互換と実点間距離の前後速度スキャン4反復を維持する。計時前は経路終点（ゴール）から始点（スタート）までの実座標差を直線助走距離とし、静止からの助走で始端速度上限を求める。助走時間は予測タイムへ含めず、終端速度は自由とする。
...
- 接触証明部品が不明・`design_confirmed=false`なら新しい大域経路を採用せず、同じ速度境界で再評価した現在のFrenet長窓最良へフォールバックする。
